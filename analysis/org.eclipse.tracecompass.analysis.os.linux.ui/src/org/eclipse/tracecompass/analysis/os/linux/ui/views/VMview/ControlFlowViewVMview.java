...
package org.eclipse.tracecompass.analysis.os.linux.ui.views.VMview;
...
//import org.eclipse.tracecompass.analysis.os.linux.core.vmanalysis.KernelAnalysisModule;
 ...
 *
 */
public class ControlFlowViewVMview extends AbstractStateSystemTimeGraphView {
     ...
     * View ID.
     */
    public static final String ID = "org.eclipse.tracecompass.analysis.os.linux.views.qemuView"; //$NON-NLS-1$
     ...
     * Constructor
     */
    public ControlFlowViewVMview() {
        super(ID, new ControlFlowPresentationProviderVMview());
    ...
    public void createPartControl(Composite parent) {
        ...
        super.getTimeGraphCombo().addTimeGraphFilterCheckActiveButton(
                new ControlFlowCheckActiveProviderVMview(Messages.ControlFlowView_checkActiveLabel, Messages.ControlFlowView_checkActiveToolTip));
        // add "Uncheck inactive" Button to TimeGraphFilterDialog
        super.getTimeGraphCombo().addTimeGraphFilterUncheckInactiveButton(
                new ControlFlowCheckActiveProviderVMview(Messages.ControlFlowView_uncheckInactiveLabel, Messages.ControlFlowView_uncheckInactiveToolTip));
    }
    ...
    private static class ControlFlowEntryComparator implements Comparator<ITimeGraphEntry> {
        ...
        public int compare(ITimeGraphEntry o1, ITimeGraphEntry o2) {
            ...
            if ((o1 instanceof ControlFlowEntryVMview) && (o2 instanceof ControlFlowEntryVMview)) {
                ControlFlowEntryVMview entry1 = (ControlFlowEntryVMview) o1;
                ControlFlowEntryVMview entry2 = (ControlFlowEntryVMview) o2;
    ...
    protected static class ControlFlowTreeLabelProvider extends TreeLabelProvider {
        ...
        public String getColumnText(Object element, int columnIndex) {
            ControlFlowEntryVMview entry = (ControlFlowEntryVMview) element;
...
protected void buildEventList(final ITmfTrace trace, final ITmfTrace parentTrace, final IProgressMonitor monitor) {
    ...
    final List<ControlFlowEntryVMview> entryList = new ArrayList<>();
    final Map<Integer, ControlFlowEntryVMview> entryMap = new HashMap<>();
    ...
    while (!complete) {
        ...
        final List<Integer> threadQuarks = ssq.getQuarks("vmName","testU1", "*"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        ...
        queryFullStates(ssq, qStart, qEnd, resolution, monitor, new IQueryHandler() {
            ...
            public void handle(List<List<ITmfStateInterval>> fullStates, List<ITmfStateInterval> prevFullState) {
                for (int threadQuark : threadQuarks) {
                    ...
                    for (List<ITmfStateInterval> fullState : fullStates) {
                        ...
                        if ((!execNameInterval.getStateValue().isNull() &&
                            ...
                            ControlFlowEntryVMview entry = entryMap.get(threadId);
                            if (entry == null) {
                                entry = new ControlFlowEntryVMview(threadQuark, parentTrace, execName, threadId, ppid, startTime, endTime);
                                entryList.add(entry);
                ...
                for (final TimeGraphEntry entry : getEntryList(ssq)) {
                    ...
                    buildStatusEvents(trace, parentTrace, ssq, fullStates, prevFullState, (ControlFlowEntryVMview) entry, monitor, qStart, qEnd);
                }
            }
        });
...
private void updateTree(List<ControlFlowEntryVMview> entryList, ITmfTrace parentTrace, ITmfStateSystem ss) {
    ...
    for (ControlFlowEntryVMview entry : entryList) {
        boolean root = (entry.getParent() == null);
        if (root && entry.getParentThreadId() > 0) {
            for (ControlFlowEntryVMview parent : entryList) {
                /*
...
private void buildStatusEvents(ITmfTrace trace, ITmfTrace parentTrace, ITmfStateSystem ss, @NonNull List<List<ITmfStateInterval>> fullStates,
        @Nullable List<ITmfStateInterval> prevFullState, ControlFlowEntryVMview entry, @NonNull IProgressMonitor monitor, long start, long end) {
    ...
    for (ITimeGraphEntry child : entry.getChildren()) {
        ...
        buildStatusEvents(trace, parentTrace, ss, fullStates, prevFullState, (ControlFlowEntryVMview) child, monitor, start, end);
    }
}
...
protected @Nullable List<ITimeEvent> getEventList(@NonNull TimeGraphEntry tgentry, ITmfStateSystem ss,
    ...
    if (!(tgentry instanceof ControlFlowEntryVMview)) {
        return eventList;
    }
    ControlFlowEntryVMview entry = (ControlFlowEntryVMview) tgentry;
    try {
...
protected void synchingToTime(long time) {
    ...
    if (selected > 0) {
        for (Object element : getTimeGraphViewer().getExpandedElements()) {
            if (element instanceof ControlFlowEntryVMview) {
                ControlFlowEntryVMview entry = (ControlFlowEntryVMview) element;
...
private ControlFlowEntryVMview findEntry(List<? extends ITimeGraphEntry> entryList, ITmfTrace trace, int threadId) {
    for (ITimeGraphEntry entry : entryList) {
        if (entry instanceof ControlFlowEntryVMview) {
            ControlFlowEntryVMview controlFlowEntry = (ControlFlowEntryVMview) entry;
