 ...
 *
 */
public class ControlFlowViewQemu extends AbstractStateSystemTimeGraphView {
     ...
     * Constructor
     */
    public ControlFlowViewQemu() {
        super(ID, new ControlFlowPresentationProviderQemu());
    ...
    public void createPartControl(Composite parent) {
        ...
        super.getTimeGraphCombo().addTimeGraphFilterCheckActiveButton(
                new ControlFlowCheckActiveProviderQemu(Messages.ControlFlowView_checkActiveLabel, Messages.ControlFlowView_checkActiveToolTip));
        // add "Uncheck inactive" Button to TimeGraphFilterDialog
        super.getTimeGraphCombo().addTimeGraphFilterUncheckInactiveButton(
                new ControlFlowCheckActiveProviderQemu(Messages.ControlFlowView_uncheckInactiveLabel, Messages.ControlFlowView_uncheckInactiveToolTip));
    }
    ...
    private static class ControlFlowEntryComparator implements Comparator<ITimeGraphEntry> {
        ...
        public int compare(ITimeGraphEntry o1, ITimeGraphEntry o2) {
            ...
            if ((o1 instanceof ControlFlowEntryQemu) && (o2 instanceof ControlFlowEntryQemu)) {
                ControlFlowEntryQemu entry1 = (ControlFlowEntryQemu) o1;
                ControlFlowEntryQemu entry2 = (ControlFlowEntryQemu) o2;
    ...
    protected static class ControlFlowTreeLabelProvider extends TreeLabelProvider {
        ...
        public String getColumnText(Object element, int columnIndex) {
            ControlFlowEntryQemu entry = (ControlFlowEntryQemu) element;
...
protected void buildEventList(final ITmfTrace trace, final ITmfTrace parentTrace, final IProgressMonitor monitor) {
    ...
    final List<ControlFlowEntryQemu> entryList = new ArrayList<>();
    final Map<Integer, ControlFlowEntryQemu> entryMap = new HashMap<>();
    ...
    while (!complete) {
        ...
        //final List<Integer> threadQuarks = ssq.getQuarks(Attributes.IO, "*"); //$NON-NLS-1$
        ...
        queryFullStates(ssq, qStart, qEnd, resolution, monitor, new IQueryHandler() {
            ...
            public void handle(List<List<ITmfStateInterval>> fullStates, List<ITmfStateInterval> prevFullState) {
                for (int threadQuark : threadQuarks) {
                    ...
                    for (List<ITmfStateInterval> fullState : fullStates) {
                        ...
                        if ((!execNameInterval.getStateValue().isNull() &&
                            ...
                            if (entry == null) {
                                entry = new ControlFlowEntryQemu(threadQuark, parentTrace, execName, threadId, ppid, startTime, endTime);
                                entryList.add(entry);
                ...
                for (final TimeGraphEntry entry : getEntryList(ssq)) {
                    ...
                    buildStatusEvents(trace, parentTrace, ssq, fullStates, prevFullState, (ControlFlowEntryQemu) entry, monitor, qStart, qEnd);
                }
            }
        });
...
private void updateTree(List<ControlFlowEntryQemu> entryList, ITmfTrace parentTrace, ITmfStateSystem ss) {
    ...
    for (ControlFlowEntryQemu entry : entryList) {
        boolean root = (entry.getParent() == null);
        if (root && entry.getParentThreadId() > 0) {
            for (ControlFlowEntryQemu parent : entryList) {
                /*
...
private void buildStatusEvents(ITmfTrace trace, ITmfTrace parentTrace, ITmfStateSystem ss, @NonNull List<List<ITmfStateInterval>> fullStates,
        @Nullable List<ITmfStateInterval> prevFullState, ControlFlowEntryQemu entry, @NonNull IProgressMonitor monitor, long start, long end) {
    ...
    for (ITimeGraphEntry child : entry.getChildren()) {
        ...
        buildStatusEvents(trace, parentTrace, ss, fullStates, prevFullState, (ControlFlowEntryQemu) child, monitor, start, end);
    }
}
...
protected @Nullable List<ITimeEvent> getEventList(@NonNull TimeGraphEntry tgentry, ITmfStateSystem ss,
    ...
    if (!(tgentry instanceof ControlFlowEntryQemu)) {
        return eventList;
    }
    ControlFlowEntryQemu entry = (ControlFlowEntryQemu) tgentry;
    try {
...
private ControlFlowEntryQemu findEntry(List<? extends ITimeGraphEntry> entryList, ITmfTrace trace, int threadId) {
    for (ITimeGraphEntry entry : entryList) {
        if (entry instanceof ControlFlowEntryQemu) {
            ControlFlowEntryQemu controlFlowEntry = (ControlFlowEntryQemu) entry;
